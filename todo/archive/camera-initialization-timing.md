# Kamera-Initialisierung stabilisieren *(abgeschlossen)*
...
## Abschluss
- `CameraView` sorgt seit `src/ui/camera.py` für eine deterministische Scrollregion und Viewport-Zentrierung.
- `ChooChooApp` setzt die Kamera vor dem ersten Render und reagiert ausschließlich auf echte Bewegungen (`src/ui/app.py`).
- `tests/ui/test_camera_initialization.py` verifiziert, dass Initialisierung und erster Tick keine unnötigen `moveto`-/`scan_dragto`-Aufrufe auslösen.
